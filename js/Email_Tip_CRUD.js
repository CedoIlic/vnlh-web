/* Email_Tip_CRUD.js – tablica + jedna kontrola (naziv)
 * API: Email_Tip_CRUD_sve.php, _upis.php, _izmjena.php, _brisanje.php
 */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Email_Tip_CRUD.html');

// ========== KONSTANTE ==========
// Email_TipCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const Email_TipCRUD = {
    Broj_Kolona: 2,
    Reload_Ikona: 0,
    CrudCssPrefix: 'email-Tip-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip e-mail adrese", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
  ...
  /* Prijevod zaglavlja tablice iz jednog ključa (naslovi zarezom: "Tip e-mail adrese, Tip"). 0-Jezik.js je sinkron →
     vnlhT je spreman ovdje; na master jeziku / bez prijevoda vraća originalne naslove. */
  if (window.vnlhTZaglavlje) {
    Email_TipCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('email_tip_crud.tablica.zaglavlje', Email_TipCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Email_TipCRUD, {
  ...
  function clearControlsFromSelection() {
    var editEl = document.getElementById('edit_tip_email');
  ...
  if (Email_TipCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        email_TipUpdate(id, naziv, tipVal, function (res) {
          if (res === 'OK') {
        ...
        email_TipAdd(naziv, tipVal, function (res) {
          if (res === 'OK') {
  ...
  if (btnIzbrisi) {
    btnIzbrisi.addEventListener('click', function () {
      ...
      function doDelete() {
        email_TipDelete(id, function (res) {
          if (res === 'OK') {
      ...
      if (otherId != null) {
        email_TipChange(otherId, 1, function (res) {
          if (res === 'OK') doDelete();
          else {
            var p = parseResponseCode(res);
            if (p && typeof MODAL_MESSAGES !== 'undefined' && MODAL_MESSAGES[p.code] && typeof window.showPorukaModal === 'function') window.showPorukaModal(p.code, p.replacements);
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('email_tip_crud.labela.naziv', 'Tip e-mail adrese')];
    }
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Email_TipCRUD.Tablica_Zaglavlje);
  }

  function email_TipAdd(naziv, tipVal, callback) {
    postFormData(API_BASE + 'Email_Tip_CRUD_upis.php', { naziv: naziv, Tip: String(tipVal) }, callback);
  }

  function email_TipUpdate(id, naziv, tipVal, callback) {
    postFormData(API_BASE + 'Email_Tip_CRUD_izmjena.php', { id: String(id), naziv: naziv, Tip: String(tipVal) }, callback);
  }

  function email_TipDelete(id, callback) {
    postFormData(API_BASE + 'Email_Tip_CRUD_brisanje.php', { id: String(id) }, callback);
  }

  function email_TipChange(id, tipVal, callback) {
    postFormData(API_BASE + 'Email_Tip_CRUD_Tip_Change.php', { id: String(id), Tip: String(tipVal) }, callback);
  }
    ...
    container.addEventListener('change', function (e) {
      ...
      if (tipVal === 0) {
        ...
        if (thisRowIsOnlyTip1) {
          ...
          email_TipChange(otherId, 1, function (res1) {
            if (res1 !== 'OK') {
              chk.checked = true;
            ...
            email_TipChange(rowId, 0, function (res2) {
              if (res2 === 'OK') {
      ...
      email_TipChange(rowId, tipVal, function (res) {
        if (res === 'OK') {
          osvjeziTablicu(function () {
            var row = container.querySelector('.kontrola-tablica__scroll tbody tr[data-row-id="' + CSS.escape(String(rowId)) + '"]');
            if (row) {
  ...
  updateCrudUpisiState();
  window.Email_TipCRUD = Email_TipCRUD;
})();
