/* Jezici_CRUD.js – tablica + jedna kontrola (naziv)
 * Uzorak: Obredi_CRUD.js
...
// ========== KONSTANTE ==========
// JeziciCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  ...
  function clearControlsFromSelection() {
    var editEl = document.getElementById('edit_jezik');
    if (editEl) {
      editEl.value = '';
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        jeziciUpdate(id, naziv, function (res) {
          if (res === 'OK') {
  ...
  function jeziciAdd(naziv, callback) {
    postFormData(API_BASE + 'Jezici_CRUD_upis.php', { naziv: naziv }, callback);
  }

  function jeziciUpdate(id, naziv, callback) {
    postFormData(API_BASE + 'Jezici_CRUD_izmjena.php', { id: String(id), naziv: naziv }, callback);
  }
