/* Telefoni_Tip_CRUD.js – tablica + jedna kontrola (naziv)
 * API: Telefoni_Tip_CRUD_sve.php, _upis.php, _izmjena.php, _brisanje.php
 */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Telefoni_Tip_CRUD.html');

// ========== KONSTANTE ==========
// Telefoni_TipCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const Telefoni_TipCRUD = {
    Broj_Kolona: 2,
    Reload_Ikona: 0,
    CrudCssPrefix: 'telefoni-Tip-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip telefonskog broja", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
  ...
  /* Prijevod zaglavlja tablice iz jednog ključa (naslovi zarezom: "Tip telefonskog broja, Tip"). 0-Jezik.js je
     sinkron → vnlhT je spreman ovdje; na master jeziku / bez prijevoda vraća originalne naslove. */
  if (window.vnlhTZaglavlje) {
    Telefoni_TipCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('telefoni_tip_crud.tablica.zaglavlje', Telefoni_TipCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Telefoni_TipCRUD, {
  ...
  function clearControlsFromSelection() {
    var editEl = document.getElementById('edit_tip_telefona');
  ...
  if (Telefoni_TipCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      /* Prvi dodani red (samo jedan red podataka) mora imati Tip 1. */
      var tipVal = jeIzmjena ? selectedTipValue : (data.length === 0 ? 1 : 0);
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        telefoni_TipUpdate(id, naziv, tipVal, function (res) {
          if (res === 'OK') {
        ...
        telefoni_TipAdd(naziv, tipVal, function (res) {
          if (res === 'OK') {
  ...
  if (btnIzbrisi) {
    btnIzbrisi.addEventListener('click', function () {
      ...
      function doDelete() {
        telefoni_TipDelete(id, function (res) {
          if (res === 'OK') {
      ...
      if (otherId != null) {
        telefoni_TipChange(otherId, 1, function (res) {
          if (res === 'OK') doDelete();
          else {
            var p = parseResponseCode(res);
            if (p && typeof MODAL_MESSAGES !== 'undefined' && MODAL_MESSAGES[p.code] && typeof window.showPorukaModal === 'function') window.showPorukaModal(p.code, p.replacements);
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('telefoni_tip_crud.labela.naziv', 'Tip telefonskog broja')];
    }
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Telefoni_TipCRUD.Tablica_Zaglavlje);
  }

  function telefoni_TipAdd(naziv, tipVal, callback) {
    postFormData(API_BASE + 'Telefoni_Tip_CRUD_upis.php', { naziv: naziv, Tip: String(tipVal) }, callback);
  }

  function telefoni_TipUpdate(id, naziv, tipVal, callback) {
    postFormData(API_BASE + 'Telefoni_Tip_CRUD_izmjena.php', { id: String(id), naziv: naziv, Tip: String(tipVal) }, callback);
  }

  function telefoni_TipDelete(id, callback) {
    postFormData(API_BASE + 'Telefoni_Tip_CRUD_brisanje.php', { id: String(id) }, callback);
  }

  function telefoni_TipChange(id, tipVal, callback) {
    postFormData(API_BASE + 'Telefoni_Tip_CRUD_Tip_Change.php', { id: String(id), Tip: String(tipVal) }, callback);
  }
  ...
  (function () {
    ...
    container.addEventListener('change', function (e) {
      ...
      if (tipVal === 0) {
        /* Nije dozvoljeno da svi redovi imaju Tip 0 – tip se može samo prebaciti na drugi red. */
        if (data.length <= 1) {
          chk.checked = true;
        ...
        if (thisRowIsOnlyTip1) {
          ...
          telefoni_TipChange(otherId, 1, function (res1) {
            if (res1 !== 'OK') {
              chk.checked = true;
            ...
            telefoni_TipChange(rowId, 0, function (res2) {
              if (res2 === 'OK') {
      ...
      telefoni_TipChange(rowId, tipVal, function (res) {
        if (res === 'OK') {
          osvjeziTablicu(function () {
            var row = container.querySelector('.kontrola-tablica__scroll tbody tr[data-row-id="' + CSS.escape(String(rowId)) + '"]');
            if (row) {
  ...
  updateCrudUpisiState();
  window.Telefoni_TipCRUD = Telefoni_TipCRUD;
})();
