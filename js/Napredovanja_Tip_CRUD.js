/* Napredovanja_Tip_CRUD.js – tablica + jedna kontrola (naziv). Tablica: napredovanja_tip. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Napredovanja_Tip_CRUD.html');

// ========== KONSTANTE ==========
// Napredovanja_TipCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const Napredovanja_TipCRUD = {
    Broj_Kolona: 1,
    Reload_Ikona: 0,
    CrudCssPrefix: 'napredovanja-tip-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip napredovanja", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
    ]
  };
  ...
  if (window.vnlhTZaglavlje) {
    Napredovanja_TipCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('napredovanja_tip_crud.tablica.zaglavlje', Napredovanja_TipCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Napredovanja_TipCRUD, {
  ...
  if (Napredovanja_TipCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        napredovanjaTipUpdate(id, naziv, function (res) {
          if (res === 'OK') {
        ...
        napredovanjaTipAdd(naziv, function (res) {
          if (res === 'OK') {
  ...
  if (btnIzbrisi) {
    btnIzbrisi.addEventListener('click', function () {
      var id = getSelectedRowId();
      if (id == null) return;
      napredovanjaTipDelete(id, function (res) {
        if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('napredovanja_tip_crud.labela.naziv', 'Tip napredovanja')];
    }
  ...
  function ucitajPodatkeTablica(callback) {
    ...
    xhr.open('GET', API_BASE + 'Napredovanja_Tip_CRUD_sve.php', true);
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Napredovanja_TipCRUD.Tablica_Zaglavlje);
  }

  function napredovanjaTipAdd(naziv, callback) {
    postFormData(API_BASE + 'Napredovanja_Tip_CRUD_upis.php', { naziv: naziv }, callback);
  }

  function napredovanjaTipUpdate(id, naziv, callback) {
    postFormData(API_BASE + 'Napredovanja_Tip_CRUD_izmjena.php', { id: String(id), naziv: naziv }, callback);
  }

  function napredovanjaTipDelete(id, callback) {
    postFormData(API_BASE + 'Napredovanja_Tip_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Napredovanja_TipCRUD = Napredovanja_TipCRUD;
})();
