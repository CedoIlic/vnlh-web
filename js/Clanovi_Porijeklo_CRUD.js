/* Clanovi_Porijeklo_CRUD.js – tablica + jedna kontrola. Tablica: clanovi_porijeklo. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Clanovi_Porijeklo_CRUD.html');

// ========== KONSTANTE ==========
// Clanovi_PorijekloCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const Clanovi_PorijekloCRUD = {
    Broj_Kolona: 1,
    Reload_Ikona: 0,
    CrudCssPrefix: 'clanovi-porijeklo-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Porijeklo člana", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
    ]
  };
  ...
  if (window.vnlhTZaglavlje) {
    Clanovi_PorijekloCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('clanovi_porijeklo_crud.tablica.zaglavlje', Clanovi_PorijekloCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Clanovi_PorijekloCRUD, {
  ...
  if (Clanovi_PorijekloCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        clanoviPorijekloUpdate(id, naziv, function (res) {
          if (res === 'OK') {
        ...
        clanoviPorijekloAdd(naziv, function (res) {
          if (res === 'OK') {
  ...
  if (btnIzbrisi) {
    btnIzbrisi.addEventListener('click', function () {
      var id = getSelectedRowId();
      if (id == null) return;
      clanoviPorijekloDelete(id, function (res) {
        if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('clanovi_porijeklo_crud.labela.naziv', 'Porijeklo člana')];
    }
  ...
  function ucitajPodatkeTablica(callback) {
    ...
    xhr.open('GET', API_BASE + 'Clanovi_Porijeklo_CRUD_sve.php', true);
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Clanovi_PorijekloCRUD.Tablica_Zaglavlje);
  }

  function clanoviPorijekloAdd(naziv, callback) {
    postFormData(API_BASE + 'Clanovi_Porijeklo_CRUD_upis.php', { naziv: naziv }, callback);
  }

  function clanoviPorijekloUpdate(id, naziv, callback) {
    postFormData(API_BASE + 'Clanovi_Porijeklo_CRUD_izmjena.php', { id: String(id), naziv: naziv }, callback);
  }

  function clanoviPorijekloDelete(id, callback) {
    postFormData(API_BASE + 'Clanovi_Porijeklo_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Clanovi_PorijekloCRUD = Clanovi_PorijekloCRUD;
})();
