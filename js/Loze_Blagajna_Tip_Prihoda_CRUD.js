/* Loze_Blagajna_Tip_Prihoda_CRUD.js – tablica (naziv + redosljed) + edit. Tablica: loze_blagajna_tip_prihoda. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Loze_Blagajna_Tip_Prihoda_CRUD.html');

// ========== KONSTANTE ==========
// Konfiguracija forme. Tablica: naziv + redosljed; id se drži kao skriveni zadnji element retka (ključ selekcije).
// =============================================================================
  const Tip_PrihodaCRUD = {
    Broj_Kolona: 2,
    Reload_Ikona: 0,
    CrudCssPrefix: 'loze-blagajna-tip-prihoda-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip prihoda", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
  ...
  if (window.vnlhTZaglavlje) {
    Tip_PrihodaCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('loze_blagajna_tip_prihoda_crud.tablica.zaglavlje', Tip_PrihodaCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Tip_PrihodaCRUD, {
  ...
  function updateCrudUpisiState() {
    ...
    // Redosljed (s labelom): enable kad ima selekcija ILI sadržaj u „Tip prihoda"; inače disable.
  ...
  if (Tip_PrihodaCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        tipUpdate(id, naziv, redosljed, function (res) {
          if (res === 'OK') {
        ...
        tipAdd(naziv, redosljed, function (res) {
          if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('loze_blagajna_tip_prihoda_crud.labela.naziv', 'Tip prihoda')];
    }
  ...
  function ucitajPodatkeTablica(callback) {
    ...
    xhr.open('GET', API_BASE + 'Loze_Blagajna_Tip_Prihoda_CRUD_sve.php', true);
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Tip_PrihodaCRUD.Tablica_Zaglavlje);
  }

  function tipAdd(naziv, redosljed, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Prihoda_CRUD_upis.php', { naziv: naziv, redosljed: redosljed }, callback);
  }

  function tipUpdate(id, naziv, redosljed, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Prihoda_CRUD_izmjena.php', { id: String(id), naziv: naziv, redosljed: redosljed }, callback);
  }

  function tipDelete(id, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Prihoda_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Loze_Blagajna_Tip_PrihodaCRUD = Tip_PrihodaCRUD;
})();
