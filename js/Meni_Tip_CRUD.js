/* Meni_Tip_CRUD.js – tablica + jedna kontrola. Tablica: meni_tip. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Meni_Tip_CRUD.html');

// ========== KONSTANTE ==========
// Meni_TipCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const Meni_TipCRUD = {
    Broj_Kolona: 1,
    Reload_Ikona: 0,
    CrudCssPrefix: 'meni-tip-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip menija", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
    ]
  };
  ...
  if (window.vnlhTZaglavlje) {
    Meni_TipCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('meni_tip_crud.tablica.zaglavlje', Meni_TipCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Meni_TipCRUD, {
  ...
  if (Meni_TipCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        meniTipUpdate(id, naziv, function (res) {
          if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('meni_tip_crud.labela.naziv', 'Tip menija')];
    }
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Meni_TipCRUD.Tablica_Zaglavlje);
  }

  function meniTipAdd(naziv, callback) {
    postFormData(API_BASE + 'Meni_Tip_CRUD_upis.php', { naziv: naziv }, callback);
  }

  function meniTipUpdate(id, naziv, callback) {
    postFormData(API_BASE + 'Meni_Tip_CRUD_izmjena.php', { id: String(id), naziv: naziv }, callback);
  }

  function meniTipDelete(id, callback) {
    postFormData(API_BASE + 'Meni_Tip_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Meni_TipCRUD = Meni_TipCRUD;
})();
