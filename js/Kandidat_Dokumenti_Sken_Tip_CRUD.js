/* Kandidat_Dokumenti_Sken_Tip_CRUD.js – tablica (naziv + redosljed) + edit. Tablica: kandidat_dokumenti_sken_tip. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Kandidat_Dokumenti_Sken_Tip_CRUD.html');
...
// =============================================================================
  const Sken_TipCRUD = {
    Broj_Kolona: 2,
    Reload_Ikona: 0,
    CrudCssPrefix: 'kandidat-dokumenti-sken-tip-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip skeniranog dokumenta", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
  ...
  if (window.vnlhTZaglavlje) {
    Sken_TipCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('kandidat_dokumenti_sken_tip_crud.tablica.zaglavlje', Sken_TipCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Sken_TipCRUD, {
  ...
  function updateCrudUpisiState() {
    ...
    // Redosljed (s labelom): enable kad ima selekcija ILI sadržaj u „Tip skeniranog dokumenta"; inače disable.
  ...
  if (Sken_TipCRUD.Reload_Ikona === 1) {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        skenTipUpdate(id, naziv, redosljed, function (res) {
          if (res === 'OK') {
        ...
        skenTipAdd(naziv, redosljed, function (res) {
          if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('kandidat_dokumenti_sken_tip_crud.labela.naziv', 'Tip skeniranog dokumenta')];
    }
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Sken_TipCRUD.Tablica_Zaglavlje);
  }

  function skenTipAdd(naziv, redosljed, callback) {
    postFormData(API_BASE + 'Kandidat_Dokumenti_Sken_Tip_CRUD_upis.php', { naziv: naziv, redosljed: redosljed }, callback);
  }

  function skenTipUpdate(id, naziv, redosljed, callback) {
    postFormData(API_BASE + 'Kandidat_Dokumenti_Sken_Tip_CRUD_izmjena.php', { id: String(id), naziv: naziv, redosljed: redosljed }, callback);
  }

  function skenTipDelete(id, callback) {
    postFormData(API_BASE + 'Kandidat_Dokumenti_Sken_Tip_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Kandidat_Dokumenti_Sken_TipCRUD = Sken_TipCRUD;
})();
