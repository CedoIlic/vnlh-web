 ...
 * Boje: edit_<col> (hex) + edit_<col>_swatch (color); nullable boje uz edit_<col>_ima.
...
(function () {
  ...
  /* ---- Boje (swatch <-> hex, nullable) ---- */
  function jeHex(s) { return /^#[0-9A-Fa-f]{6}$/.test(String(s || '')); }

  function primijeniBojaIma(f) {
    if (!f.nullable) return;
    var ima = byId('edit_' + f.col + '_ima');
    var hex = elOf(f);
    var sw = byId('edit_' + f.col + '_swatch');
    var on = !!(ima && ima.checked);
    if (hex) hex.disabled = !on;
    if (sw) sw.disabled = !on;
    if (on && hex && trim(hex.value) === '' && sw) { hex.value = String(sw.value || '').toUpperCase(); }
    if (!on && hex) hex.value = '';
  }
  ...
  function initBoja(f) {
    ...
    if (sw) sw.addEventListener('input', function () {
      if (hex) hex.value = String(sw.value || '').toUpperCase();
      onFormPromjena();
    });
    if (hex) hex.addEventListener('input', function () {
      if (sw && jeHex(hex.value)) sw.value = hex.value;
      onFormPromjena();
    });
    if (f.nullable) {
      var ima = byId('edit_' + f.col + '_ima');
      if (ima) ima.addEventListener('change', function () { primijeniBojaIma(f); onFormPromjena(); });
    }
  }

  /* ---- Okvir dominira nad pozadinom (vizualni signal + onemogući pozadinu) ---- */
  var POZADINA_IDS = ['edit_boja_pozadine_ima', 'edit_boja_pozadine', 'edit_boja_pozadine_swatch',
  ...
  function azurirajOkvirPozadina() {
    ...
    if (!akt) primijeniBojaIma({ col: 'boja_pozadine', nullable: true });
  }
  ...
  function azurirajDisableTaba() {
    ...
    if (imaFont) {
      FIELDS.forEach(function (f) { if (f.type === 'color' && f.nullable) primijeniBojaIma(f); });
  ...
  function popuniIzObjekta(o) {
    FIELDS.forEach(function (f) {
      ...
      if (f.type === 'check') {
        ...
        if (f.nullable) { var ima = byId('edit_' + f.col + '_ima'); if (ima) ima.checked = has; }
        el.value = has ? String(v).toUpperCase() : (f.nullable ? '' : (f.def || ''));
        var sw = byId('edit_' + f.col + '_swatch');
        if (sw && jeHex(el.value)) sw.value = el.value;
        if (f.nullable) primijeniBojaIma(f);
      } else if (f.type === 'num') {
        el.value = (v != null && v !== '')
  ...
  function clearForm() {
    FIELDS.forEach(function (f) {
      ...
      if (f.type === 'check') { el.checked = false; }
        ...
        if (f.nullable) { var ima = byId('edit_' + f.col + '_ima'); if (ima) ima.checked = false; el.value = ''; primijeniBojaIma(f); }
        else { el.value = f.def; var sw = byId('edit_' + f.col + '_swatch'); if (sw) sw.value = f.def; }
  ...
  function sakupiParams() {
    ...
    FIELDS.forEach(function (f) {
      ...
      if (f.type === 'check') { p[f.col] = el.checked ? '1' : '0'; }
        ...
        if (f.nullable) { var ima = byId('edit_' + f.col + '_ima'); p[f.col] = (ima && ima.checked) ? trim(el.value) : ''; }
  ...
  FIELDS.forEach(function (f) { if (f.type === 'color') initBoja(f); });
  ucitajFontove();
  ucitajPodatkeTablica(function (rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, PDF_StiloviCRUD.Tablica_Zaglavlje);
  });
