...
(function () {
  ...
  function ucitajSveSelekte(cb) {
    ...
    xhrGet(API + 'PDF_Whitelist_CRUD_sve.php', function (t) {
      try { var a = JSON.parse(t || '[]'); a.forEach(function (o) { mapaIzvor[String(o.id)] = o; }); napuniSelekt('st_izvor', a, null, true); napuniSelekt('st_preko_izvor', a, null, true); napuniSelekt('st_uvjet_izvor', a, null, true); napuniSelekt('st_uvjet_preko_izvor', a, null, true); } catch (e) {}
      gotovo();
    });
    xhrGet(API + 'PDF_Stilovi_CRUD_sve.php', function (t) {
      try { var a = JSON.parse(t || '[]'); a.forEach(function (o) { mapaParagraf[String(o.id)] = o; }); napuniSelekt('st_paragraf_id', a, null, true); napuniSelekt('st_podatak_paragraf', a, null, true); napuniSelekt('edit_broj_stranice_paragraf_id', a, null, true); napuniSelekt('edit_extra_prored_paragraf_id', a, null, true); } catch (e) {}
      gotovo();
    });
  ...
  function stavkaRed(s, idx) {
    ...
    var stil = '';
    if ((s.vrsta === 'tekst' || s.vrsta === 'linije') && s.paragraf_id && mapaParagraf[String(s.paragraf_id)]) stil = mapaParagraf[String(s.paragraf_id)].naziv;
  ...
  function prikaziStavkaPopup(s) {
    zatvoriStavkaPopup();
    var TIP = { staticki: 'Statički (fiksni ID)', dinamicki: 'Dinamički (iz konteksta)', po_vrijednosti: 'Po vrijednosti', korisnicki: 'Korisnički tekst', relacija_broj: 'Relacija — broj (mapa)', relacija_lista: 'Relacija — lista naziva', relacija_redak: 'Relacija — redak (predložak)', relacija_grupe: 'Relacija — grupe (po tipu)', relacija_csv: 'Relacija — CSV kolona (predložak)', tablica_glasanja: 'Tablica — glasanja', linije: 'Linije (ručno popunjavanje)' };
    ...
    if (parseInt(s.zadrzi_svoj_stil, 10)) content.appendChild(popupRed('Vlastiti stil u redu', 'da'));
    if (parseInt(s.prijelom_prije, 10)) content.appendChild(popupRed('Prijelom stranice prije', 'da'));
    if (parseInt(s.prijelom_poslije, 10)) content.appendChild(popupRed('Prijelom stranice poslije', 'da'));
    if (s.prazno_nacin && s.prazno_nacin !== 'placeholder') content.appendChild(popupRed('Kad je prazno', s.prazno_nacin));
    if (s.uvjet_izvor_id) {
      var uiz = mapaIzvor[String(s.uvjet_izvor_id)];
      var upz = s.uvjet_preko_izvor_id ? mapaIzvor[String(s.uvjet_preko_izvor_id)] : null;
      var upTxt = s.uvjet_preko_izvor_id ? ('  (preko ' + (upz ? (upz.tablica + '.' + upz.kolona) : ('#' + s.uvjet_preko_izvor_id)) + ')') : '';
      content.appendChild(popupRed('Uvjet ispisa', (uiz ? (uiz.tablica + '.' + uiz.kolona) : ('#' + s.uvjet_izvor_id)) + ' ' + (s.uvjet_operator || '=') + ' ' + (s.uvjet_vrijednost != null ? s.uvjet_vrijednost : '') + (s.uvjet_kontekst_kljuc ? ('  [' + s.uvjet_kontekst_kljuc + ']') : '') + upTxt));
    }
    if (s.skupina) content.appendChild(popupRed('Skupina', s.skupina));
    if (s.prefiks) content.appendChild(popupRed('Prefiks', s.prefiks));
    if (s.sufiks) content.appendChild(popupRed('Sufiks', s.sufiks));
  ...
  function azurirajVidljivostStavke() {
    ...
    if (vrsta === 'slika' && (tip === 'korisnicki' || tip === 'relacija_broj' || tip === 'relacija_lista' || tip === 'relacija_redak' || tip === 'relacija_grupe' || tip === 'relacija_csv')) { setVal('st_izvor_tip', 'staticki'); refreshSelect('st_izvor_tip'); tip = 'staticki'; }
    var korisnicki = (tip === 'korisnicki');
    var relacija = (tip === 'relacija_broj' || tip === 'relacija_lista' || tip === 'relacija_redak' || tip === 'relacija_grupe' || tip === 'relacija_csv');
    var relacijaLista = (tip === 'relacija_lista');
    var relacijaRedak = (tip === 'relacija_redak');
    var relacijaGrupe = (tip === 'relacija_grupe');
    var relacijaCsv = (tip === 'relacija_csv');
    var predlozakVidljiv = (relacijaRedak || relacijaGrupe || relacijaCsv);   /* redak-predložak / predložak imena / CSV predložak */
    ...
    byId('polje_bez_kraja').hidden = (vrsta !== 'tekst');   /* grupa spajanja/prijeloma — samo tekst-stavke */
    byId('polje_prazno').hidden = (vrsta !== 'tekst');            /* ponašanje na prazno — samo tekst */
    /* Dužina crte samo kad je izabrano „Linija za ručni upis". */
    if (byId('polje_prazno_linija_mm')) byId('polje_prazno_linija_mm').hidden = (vrsta !== 'tekst' || val('st_prazno_nacin') !== 'linija');
    /* Usporedba uvjeta ima smisla tek kad je izabrano polje uvjeta. */
    if (byId('polje_uvjet_red')) byId('polje_uvjet_red').hidden = (trim(val('st_uvjet_izvor')) === '');
    if (byId('polje_uvjet_preko_izvor')) byId('polje_uvjet_preko_izvor').hidden = (trim(val('st_uvjet_izvor')) === '');
    byId('polje_skupina').hidden = (vrsta !== 'tekst');          /* skupina (povezane stavke) — samo tekst */
    byId('polje_prefiks_sufiks').hidden = (vrsta !== 'tekst');   /* prefiks/sufiks — samo tekst */
    ...
    if (byId('polje_podatak_paragraf')) byId('polje_podatak_paragraf').hidden = !relacijaCsv;   /* stil podatka samo za relacija_csv */
    byId('polje_mapa').hidden = korisnicki || relacijaLista || relacijaRedak || relacijaCsv;   /* mapa: ne za korisnički/listu/redak/csv; relacija_grupe je koristi za labelu po broju (bez grupe) */
    ...
    /* Linije (ručno popunjavanje): vlastiti panel; paragraf = stil labele/linija (kao i tekst). */
    var linije = (vrsta === 'linije');
    byId('polje_linije').hidden = !linije;
    byId('polje_paragraf').hidden = (vrsta !== 'tekst' && !linije);
    if (linije) { byId('polje_izvor').hidden = true; byId('polje_mapa').hidden = true; }   /* linije nemaju izvor ni mapu */
    var istiRed = byId('st_labela_u_istom_redu');
    var prvaVidljiv = linije && istiRed && istiRed.checked;
    byId('polje_prva_linija').hidden = !prvaVidljiv;
    byId('red_prva_linija_mm').hidden = !(prvaVidljiv && val('st_prva_linija_nacin') !== 'margina');
  }
  ...
  function popuniStavkaEdit(s) {
    ...
    setVal('st_podatak_paragraf', s.podatak_paragraf_id || ''); refreshSelect('st_podatak_paragraf');
    ...
    var zss = byId('st_zadrzi_svoj_stil'); if (zss) zss.checked = !!(parseInt(s.zadrzi_svoj_stil, 10));
    var pp = byId('st_prijelom_prije'); if (pp) pp.checked = !!(parseInt(s.prijelom_prije, 10));
    var ppo = byId('st_prijelom_poslije'); if (ppo) ppo.checked = !!(parseInt(s.prijelom_poslije, 10));
    setVal('st_prazno_nacin', s.prazno_nacin || 'placeholder'); refreshSelect('st_prazno_nacin');
    setVal('st_prazno_linija_mm', s.prazno_linija_mm != null ? s.prazno_linija_mm : '');
    setVal('st_uvjet_izvor', s.uvjet_izvor_id || ''); refreshSelect('st_uvjet_izvor');
    setVal('st_uvjet_preko_izvor', s.uvjet_preko_izvor_id || ''); refreshSelect('st_uvjet_preko_izvor');
    setVal('st_uvjet_kontekst_kljuc', s.uvjet_kontekst_kljuc || '');
    setVal('st_uvjet_operator', s.uvjet_operator || '='); refreshSelect('st_uvjet_operator');
    setVal('st_uvjet_vrijednost', s.uvjet_vrijednost != null ? s.uvjet_vrijednost : '');
    setVal('st_skupina', s.skupina != null ? s.skupina : '');
    setVal('st_prefiks', s.prefiks != null ? s.prefiks : '');
    setVal('st_sufiks', s.sufiks != null ? s.sufiks : '');
    /* Linije (vrsta=linije): labela je u literal_tekst; ostali parametri u vlastitim stupcima. */
    setVal('st_linije_labela', (s.vrsta === 'linije' && s.literal_tekst != null) ? s.literal_tekst : '');
    setVal('st_broj_linija', s.broj_linija != null ? s.broj_linija : '');
    setVal('st_stil_linije', s.stil_linije || 'crtkano'); refreshSelect('st_stil_linije');
    setVal('st_linija_debljina', s.linija_debljina_mm != null ? s.linija_debljina_mm : '');
  ...
  function procitajFormuUStavku(s) {
    ...
    var jeRelacija = (s.izvor_tip === 'relacija_broj' || s.izvor_tip === 'relacija_lista' || s.izvor_tip === 'relacija_redak' || s.izvor_tip === 'relacija_grupe' || s.izvor_tip === 'relacija_csv');
    ...
    s.redak_predlozak = (s.izvor_tip === 'relacija_redak' || s.izvor_tip === 'relacija_grupe' || s.izvor_tip === 'relacija_csv') ? (trim(val('st_redak_predlozak')) || null) : null;
    var lbEl = byId('st_labela_bold'); s.labela_bold = (s.izvor_tip === 'relacija_grupe' && lbEl && lbEl.checked) ? 1 : 0;
    /* mapa: null za korisnicki/relacija_lista/relacija_redak/relacija_csv; inače (staticki/dinamicki/po_vrijednosti/relacija_broj) iz polja */
    s.mapa_vrijednosti = (s.izvor_tip === 'korisnicki' || s.izvor_tip === 'relacija_lista' || s.izvor_tip === 'relacija_redak' || s.izvor_tip === 'relacija_csv') ? null : (trim(val('st_mapa_vrijednosti')) || null);
    ...
    var jeLinije = (s.vrsta === 'linije');
    /* literal_tekst: korisnički tekst (tekst) ILI labela (linije, opcionalno). */
    s.literal_tekst = (s.izvor_tip === 'korisnicki') ? trim(val('st_literal_tekst')) : (jeLinije ? (trim(val('st_linije_labela')) || null) : null);
    s.paragraf_id = trim(val('st_paragraf_id')) ? parseInt(val('st_paragraf_id'), 10) : null;
    /* Parametri linija (samo vrsta=linije; inače null / 0). */
    s.broj_linija = jeLinije ? (parseInt(trim(val('st_broj_linija')), 10) > 0 ? parseInt(trim(val('st_broj_linija')), 10) : 1) : null;
    s.stil_linije = jeLinije ? (val('st_stil_linije') || 'crtkano') : null;
    var ldRaw = trim(val('st_linija_debljina')).replace(',', '.'); var ldNum = ldRaw !== '' ? parseFloat(ldRaw) : NaN;
    s.linija_debljina_mm = (jeLinije && !isNaN(ldNum) && ldNum > 0) ? ldNum : null;
    var istiRedEl2 = byId('st_labela_u_istom_redu'); s.labela_u_istom_redu = (jeLinije && istiRedEl2 && istiRedEl2.checked) ? 1 : 0;
    ...
    var zss = byId('st_zadrzi_svoj_stil'); s.zadrzi_svoj_stil = (s.vrsta === 'tekst' && zss && zss.checked) ? 1 : 0;
    var pp = byId('st_prijelom_prije'); s.prijelom_prije = (s.zona === 'tijelo' && !s.okvir_id && pp && pp.checked) ? 1 : 0;
    var ppo = byId('st_prijelom_poslije'); s.prijelom_poslije = (s.zona === 'tijelo' && !s.okvir_id && ppo && ppo.checked) ? 1 : 0;
    s.prazno_nacin = (s.vrsta === 'tekst') ? (val('st_prazno_nacin') || 'placeholder') : 'placeholder';
    /* Dužina crte ima smisla samo uz „linija"; inače se ne pamti. */
    s.prazno_linija_mm = (s.prazno_nacin === 'linija' && trim(val('st_prazno_linija_mm')) !== '') ? parseFloat(val('st_prazno_linija_mm')) : null;
    /* Uvjetni ispis: bez izabranog polja nema uvjeta (ostalo se ne pamti). */
    s.uvjet_izvor_id = trim(val('st_uvjet_izvor')) ? parseInt(val('st_uvjet_izvor'), 10) : null;
    s.uvjet_preko_izvor_id = (s.uvjet_izvor_id && trim(val('st_uvjet_preko_izvor'))) ? parseInt(val('st_uvjet_preko_izvor'), 10) : null;
    s.uvjet_kontekst_kljuc = s.uvjet_izvor_id ? (trim(val('st_uvjet_kontekst_kljuc')) || null) : null;
    s.uvjet_operator = s.uvjet_izvor_id ? (val('st_uvjet_operator') === '<>' ? '<>' : '=') : '=';
    s.uvjet_vrijednost = s.uvjet_izvor_id ? (trim(val('st_uvjet_vrijednost')) || null) : null;
    var skRaw = parseInt(trim(val('st_skupina')), 10);
    s.skupina = (s.vrsta === 'tekst' && !isNaN(skRaw) && skRaw > 0) ? skRaw : null;
    s.prefiks = (s.vrsta === 'tekst') ? (trim(val('st_prefiks')) || null) : null;
    s.sufiks = (s.vrsta === 'tekst') ? (trim(val('st_sufiks')) || null) : null;
  }
  ...
  STAVKA_POLJA.forEach(function (id) {
    ...
    e.addEventListener(ev, function () {
      if (id === 'st_izvor_tip' || id === 'st_vrsta' || id === 'st_izvor' || id === 'st_lista_nacin' || id === 'st_labela_u_istom_redu' || id === 'st_prva_linija_nacin' || id === 'st_prazno_nacin' || id === 'st_uvjet_izvor') azurirajVidljivostStavke();
  ...
  /* ---- Popup uputa za grupu checkboxova reda (spajanje / vlastiti stil / sakrij / prijelom) ---- */
  (function () {
    var m = byId('cbPomocModal'); if (!m) return;
    function otvori() { m.setAttribute('aria-hidden', 'false'); m.classList.add('kontrola-modal--open'); }
    function zatvori() { m.setAttribute('aria-hidden', 'true'); m.classList.remove('kontrola-modal--open'); }
    var bp = byId('btnCbPomoc'); if (bp) bp.addEventListener('click', otvori);
    var ok = byId('btnCbPomocOk'); if (ok) ok.addEventListener('click', zatvori);
    var ov = byId('cbPomocModal_overlay'); if (ov) ov.addEventListener('click', zatvori);
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape' && m.getAttribute('aria-hidden') === 'false') zatvori(); });
    var dlg = m.querySelector('.kontrola-modal__dialog'), hdr = byId('cbPomocModal_header');
    if (dlg && hdr && typeof KontroleModalDrag === 'function') KontroleModalDrag(dlg, hdr);
  })();
  ...
  (function () {
    ...
    function otvori() { m.setAttribute('aria-hidden', 'false'); m.classList.add('kontrola-modal--open'); }
    function zatvori() { m.setAttribute('aria-hidden', 'true'); m.classList.remove('kontrola-modal--open'); }
    var bp = byId('btnPrefSufPomoc'); if (bp) bp.addEventListener('click', otvori);
    var ok = byId('btnPrefSufPomocOk'); if (ok) ok.addEventListener('click', zatvori);
    var ov = byId('prefSufPomocModal_overlay'); if (ov) ov.addEventListener('click', zatvori);
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape' && m.getAttribute('aria-hidden') === 'false') zatvori(); });
    var dlg = m.querySelector('.kontrola-modal__dialog'), hdr = byId('prefSufPomocModal_header');
    if (dlg && hdr && typeof KontroleModalDrag === 'function') KontroleModalDrag(dlg, hdr);
  })();
  ...
  function stavkaIzReda(r) {
    return {
      ...
      podatak_paragraf_id: r.podatak_paragraf_id ? parseInt(r.podatak_paragraf_id, 10) : null,
      tablica_stil_id: r.tablica_stil_id ? parseInt(r.tablica_stil_id, 10) : null,
      ...
      zadrzi_svoj_stil: (parseInt(r.zadrzi_svoj_stil, 10) === 1) ? 1 : 0,
      prijelom_prije: (parseInt(r.prijelom_prije, 10) === 1) ? 1 : 0,
      prijelom_poslije: (parseInt(r.prijelom_poslije, 10) === 1) ? 1 : 0,
      prazno_nacin: (['placeholder', 'crtica', 'izostavi', 'linija'].indexOf(r.prazno_nacin) >= 0) ? r.prazno_nacin : 'placeholder',
      prazno_linija_mm: (r.prazno_linija_mm != null && r.prazno_linija_mm !== '') ? parseFloat(r.prazno_linija_mm) : null,
      uvjet_izvor_id: r.uvjet_izvor_id ? parseInt(r.uvjet_izvor_id, 10) : null,
      uvjet_preko_izvor_id: r.uvjet_preko_izvor_id ? parseInt(r.uvjet_preko_izvor_id, 10) : null,
      uvjet_kontekst_kljuc: (r.uvjet_kontekst_kljuc != null && r.uvjet_kontekst_kljuc !== '') ? r.uvjet_kontekst_kljuc : null,
      uvjet_operator: (r.uvjet_operator === '<>') ? '<>' : '=',
      uvjet_vrijednost: (r.uvjet_vrijednost != null && r.uvjet_vrijednost !== '') ? r.uvjet_vrijednost : null,
      skupina: (r.skupina != null && r.skupina !== '' && parseInt(r.skupina, 10) > 0) ? parseInt(r.skupina, 10) : null,
      prefiks: (r.prefiks != null && r.prefiks !== '') ? r.prefiks : null,
      sufiks: (r.sufiks != null && r.sufiks !== '') ? r.sufiks : null,
      ...
      broj_linija: (r.broj_linija != null && r.broj_linija !== '') ? parseInt(r.broj_linija, 10) : null,
      stil_linije: r.stil_linije || null,
  ...
  function osvjeziEditSelekte(cb) {
    ...
    xhrGet(API + 'PDF_Whitelist_CRUD_sve.php', function (t) {
      try {
        ...
        var v1 = val('st_izvor'), v2 = val('st_preko_izvor'), v3 = val('st_uvjet_izvor'), v4 = val('st_uvjet_preko_izvor');
        napuniSelekt('st_izvor', a, null, true); napuniSelekt('st_preko_izvor', a, null, true); napuniSelekt('st_uvjet_izvor', a, null, true); napuniSelekt('st_uvjet_preko_izvor', a, null, true);
        setVal('st_izvor', v1); refreshSelect('st_izvor'); setVal('st_preko_izvor', v2); refreshSelect('st_preko_izvor'); setVal('st_uvjet_izvor', v3); refreshSelect('st_uvjet_izvor'); setVal('st_uvjet_preko_izvor', v4); refreshSelect('st_uvjet_preko_izvor');
      } catch (e) {}
      gotovo();
    });
    xhrGet(API + 'PDF_Stilovi_CRUD_sve.php', function (t) {
      try {
        ...
        var v3 = val('st_paragraf_id'), v4 = val('edit_broj_stranice_paragraf_id'), v5 = val('edit_extra_prored_paragraf_id'), v6 = val('st_podatak_paragraf');
        napuniSelekt('st_paragraf_id', a, null, true); napuniSelekt('st_podatak_paragraf', a, null, true); napuniSelekt('edit_broj_stranice_paragraf_id', a, null, true); napuniSelekt('edit_extra_prored_paragraf_id', a, null, true);
        setVal('st_paragraf_id', v3); refreshSelect('st_paragraf_id'); setVal('st_podatak_paragraf', v6); refreshSelect('st_podatak_paragraf'); setVal('edit_broj_stranice_paragraf_id', v4); refreshSelect('edit_broj_stranice_paragraf_id'); setVal('edit_extra_prored_paragraf_id', v5); refreshSelect('edit_extra_prored_paragraf_id');
      } catch (e) {}
      gotovo();
    });
    xhrGet(API + 'PDF_Stilovi_Slike_CRUD_sve.php', function (t) {
  ...
  /* Nova stranica „Prije" / „Poslje" su isključivi: čekiranje jednog odčekira drugi. */
  (function () {
    var pp = byId('st_prijelom_prije'), ppo = byId('st_prijelom_poslije');
    if (pp) pp.addEventListener('change', function () { if (pp.checked && ppo) ppo.checked = false; });
    if (ppo) ppo.addEventListener('change', function () { if (ppo.checked && pp) pp.checked = false; });
  })();
  ...
  function validirajStavke(lista) {
    ...
    (lista || []).forEach(function (s, i) {
      ...
      } else if (s.vrsta === 'tablica') {
        if (!(parseInt(s.tablica_stil_id, 10) > 0)) manjka.push('Stil tablice');
      } else if (s.vrsta === 'linije') {
        if (!(parseInt(s.paragraf_id, 10) > 0)) manjka.push('Stil (paragraf)');
      } else {
        manjka.push('Vrsta');
      }
      var tip = s.izvor_tip;
      if (tip === 'staticki') {
        ...
        /* Iznimka: uz „Kad je prazno = Linija" tekst MORA biti prazan — prazna vrijednost je
           upravo ono što pokreće crtu za ručni upis. */
        if (!trim(s.literal_tekst || '') && s.prazno_nacin !== 'linija') manjka.push('Korisnički tekst');
        ...
        if (!trim(s.kontekst_kljuc || '')) manjka.push('Ključ konteksta');
      } else if (tip === 'relacija_redak' || tip === 'relacija_grupe' || tip === 'relacija_csv') {
        if (s.vrsta !== 'tekst') manjka.push('Relacija je dozvoljena samo za tekst');
        if (!(parseInt(s.relacija_id, 10) > 0)) manjka.push('Relacija');
        if (!trim(s.kontekst_kljuc || '')) manjka.push('Ključ konteksta');
        if (!trim(s.redak_predlozak || '')) manjka.push(tip === 'relacija_grupe' ? 'Predložak imena' : 'Redak-predložak');
      } else if (tip === 'tablica_glasanja') {
        if (s.vrsta !== 'tablica') manjka.push('Tablica glasanja je dozvoljena samo za vrstu tablica');
        if (!trim(s.kontekst_kljuc || '')) manjka.push('Ključ konteksta');
      } else if (tip === 'linije') {
        if (s.vrsta !== 'linije') manjka.push('Linije su dozvoljene samo za vrstu linije');
      } else {
