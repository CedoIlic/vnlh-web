/* Loze_Blagajna_Tip_Troska_CRUD.js – tablica (naziv + redosljed) + edit. Tablica: loze_blagajna_tip_troska. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Loze_Blagajna_Tip_Troska_CRUD.html');

// ========== KONSTANTE ==========
// Konfiguracija forme. Tablica: naziv + redosljed; id se drži kao skriveni zadnji element retka (ključ selekcije).
// =============================================================================
  const Tip_TroskaCRUD = {
    Broj_Kolona: 2,
    Reload_Ikona: 0,
    CrudCssPrefix: 'loze-blagajna-tip-troska-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Tip troška", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
  ...
  if (window.vnlhTZaglavlje) {
    Tip_TroskaCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('loze_blagajna_tip_troska_crud.tablica.zaglavlje', Tip_TroskaCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', Tip_TroskaCRUD, {
  ...
  function updateCrudUpisiState() {
    ...
    // Redosljed (s labelom): enable kad ima selekcija ILI sadržaj u „Tip troška"; inače disable.
  ...
  if (Tip_TroskaCRUD.Reload_Ikona === 1) {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('loze_blagajna_tip_troska_crud.labela.naziv', 'Tip troška')];
    }
  ...
  function ucitajPodatkeTablica(callback) {
    ...
    xhr.open('GET', API_BASE + 'Loze_Blagajna_Tip_Troska_CRUD_sve.php', true);
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, Tip_TroskaCRUD.Tablica_Zaglavlje);
  }

  function tipAdd(naziv, redosljed, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Troska_CRUD_upis.php', { naziv: naziv, redosljed: redosljed }, callback);
  }

  function tipUpdate(id, naziv, redosljed, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Troska_CRUD_izmjena.php', { id: String(id), naziv: naziv, redosljed: redosljed }, callback);
  }

  function tipDelete(id, callback) {
    postFormData(API_BASE + 'Loze_Blagajna_Tip_Troska_CRUD_brisanje.php', { id: String(id) }, callback);
  }
  ...
  updateCrudUpisiState();
  window.Loze_Blagajna_Tip_TroskaCRUD = Tip_TroskaCRUD;
})();
