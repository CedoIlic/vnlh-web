/* Drzave_CRUD.js – tablica + jedna kontrola (naziv). Tablica: drzave. */
// @ts-nocheck
(function () {
  'use strict';
  if (typeof vnlhUcitajPravaCrud === 'function') vnlhUcitajPravaCrud('Drzave_CRUD.html');

// ========== KONSTANTE ==========
// DrzaveCRUD – jedinstveni objekt s konfiguracijom forme.
...
// =============================================================================
  const DrzaveCRUD = {
    Broj_Kolona: 1,
    Reload_Ikona: 0,
    CrudCssPrefix: 'drzave-crud',
    Tablica_Zaglavlje: [
      { key: "naziv", title: "Naziv države", SQL_Naziv: "naziv", sortable: 1, sortable_icon: 0, type: "t", width: 0, suffix: "", align: "L", row_align: "L", mobitel_prikaz: 1 },
    ]
  };
  ...
  if (window.vnlhTZaglavlje) {
    DrzaveCRUD.Tablica_Zaglavlje = window.vnlhTZaglavlje('drzave_crud.tablica.zaglavlje', DrzaveCRUD.Tablica_Zaglavlje);
  }

  CommonCRUD.initTablica('tablicaContainer', DrzaveCRUD, {
  ...
  if (btnUpisi) {
    btnUpisi.addEventListener('click', function () {
      ...
      if (jeIzmjena) {
        var id = getSelectedRowId();
        if (id == null) return;
        drzaveUpdate(id, naziv, function (res) {
          if (res === 'OK') {
  ...
  function prikaziGresku(res) {
    ...
    if (p.code === '002') {
      var tt = window.vnlhT || function (k, f) { return f != null ? f : k; };
      repl = [tt('drzave_crud.labela.naziv', 'Naziv države')];
    }
  ...
  function ucitajPodatkeTablica(callback) {
    ...
    xhr.open('GET', API_BASE + 'Drzave_CRUD_sve.php', true);
  ...
  function setDataTablica(rows) {
    CommonCRUD.setDataTablica(tablicaApi, 'tablicaContainer', rows, DrzaveCRUD.Tablica_Zaglavlje);
  }

  function drzaveAdd(naziv, callback) {
    postFormData(API_BASE + 'Drzave_CRUD_upis.php', { naziv: naziv }, callback);
  }

  function drzaveUpdate(id, naziv, callback) {
    postFormData(API_BASE + 'Drzave_CRUD_izmjena.php', { id: String(id), naziv: naziv }, callback);
  }

  function drzaveDelete(id, callback) {
    postFormData(API_BASE + 'Drzave_CRUD_brisanje.php', { id: String(id) }, callback);
  }
