...
// Loze_Blagajna_Tip_Troska_CRUD_upis.php — novi tip troška (POST: naziv, redosljed).
...
try {
    $stmt = $mysqli->prepare("SELECT id FROM loze_blagajna_tip_troska WHERE LOWER(naziv) = LOWER(?)");
    ...
    $stmt->close();
    $stmt = $mysqli->prepare("INSERT INTO loze_blagajna_tip_troska (naziv, redosljed) VALUES (?, ?)");
    if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
