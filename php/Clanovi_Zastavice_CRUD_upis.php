...
$stmt = $mysqli->prepare("SELECT id FROM clanovi_zastavice WHERE id = ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("INSERT INTO clanovi_zastavice (id, naziv, opis, boja, aktivnost) VALUES (?, ?, ?, ?, ?)");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
$stmt->bind_param("isssi", $id, $naziv, $opis, $boja, $aktivnost);
$stmt->execute();
echo 'OK';
$stmt->close();
$mysqli->close();
