...
if ($redosljed !== '' && ($redosljed_int < 1 || $redosljed_int > 99)) { echo '105'; exit; }
$stmt = $mysqli->prepare("SELECT id FROM loze_tip WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE loze_tip SET naziv = ?, redosljed = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
