...
// Loze_Blagajna_Tip_Troska_CRUD_izmjena.php — izmjena tipa troška (POST: id, naziv, redosljed).
...
$stmt = $mysqli->prepare("SELECT id FROM loze_blagajna_tip_troska WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE loze_blagajna_tip_troska SET naziv = ?, redosljed = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
