...
$stmt = $mysqli->prepare(
    'SELECT id FROM radovi_drzave_gostiju WHERE LOWER(COALESCE(naziv, \'\')) = ? AND id <> ? LIMIT 1'
);
if (!$stmt) {
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE radovi_drzave_gostiju SET naziv = ?, redosljed = ? WHERE id = ?");
if (!$stmt) {
    echo '200,' . $mysqli->errno;
    exit;
}
