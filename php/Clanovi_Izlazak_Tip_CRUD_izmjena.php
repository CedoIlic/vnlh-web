...
$stmt = $mysqli->prepare("SELECT id FROM clanovi_izlazak_tip WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE clanovi_izlazak_tip SET naziv = ?, redosljed = ?, kljuc = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
$stmt->bind_param("siii", $naziv_raw, $redosljed, $kljuc, $id);
