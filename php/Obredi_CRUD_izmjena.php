...
// =====================================================
// Obredi_CRUD_izmjena.php
// Izmjena obreda (naziv); duplikati nisu dopušteni
// =====================================================
//
...
$stmt = $mysqli->prepare(
    "SELECT id FROM obredi WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1"
);
if (!$stmt) {
...
$stmt = $mysqli->prepare(
    "UPDATE obredi SET naziv = ? WHERE id = ?"
);
if (!$stmt) {
    echo '200,' . $mysqli->errno;
    exit;
