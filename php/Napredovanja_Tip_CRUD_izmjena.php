...
$stmt = $mysqli->prepare("SELECT id FROM napredovanja_tip WHERE LOWER(COALESCE(naziv,'')) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE napredovanja_tip SET naziv = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
