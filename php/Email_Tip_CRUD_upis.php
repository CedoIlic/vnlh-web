...
// Email_Tip_CRUD_upis.php – dodavanje tipa e-mail adrese
...
try {
    $stmt = $mysqli->prepare("SELECT id FROM email_tip WHERE LOWER(naziv) = LOWER(?)");
    ...
    $stmt->close();
    $stmt = $mysqli->prepare("INSERT INTO email_tip (naziv, `Tip`) VALUES (?, ?)");
    if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
