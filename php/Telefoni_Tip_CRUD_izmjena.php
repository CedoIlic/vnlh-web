...
$stmt = $mysqli->prepare("SELECT id FROM telefoni_tip WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE telefoni_tip SET naziv = ?, `Tip` = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
