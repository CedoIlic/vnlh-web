...
try {
    ...
    if (!$id_domacin || !$id_stupanj || !$id_tip_radova) {
    ...
    // INSERT zapisnik_sa_radova
    $stmt = $mysqli->prepare("
        INSERT INTO zapisnik_sa_radova
            (id_domacin, id_obred, id_stupanj, id_tip_radova, datum_radova,
             ...
             sazetak, zapisnik, upisao, vrijeme_upisa)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    $stmt->bind_param(
        'iiiisiiiiiiiissi',
        ...
        $ovjera_poslije_tajnik, $ovjera_poslije_tajnik_id,
        $ovjera_poslije_govornik, $ovjera_poslije_govornik_id,
        $sazetak, $zapisnik, $upisao
    );
    $stmt->execute();
    $id_zapisnika = (int)$mysqli->insert_id;
    $stmt->close();

    // INSERT zapisnik_sa_radova_loze_ucesnice
    ...
    if ($loze) {
        ...
        foreach ($loze as $idL) {
            $idL = (int)$idL;
            if (!$idL) continue;
            $stmt->bind_param('ii', $id_zapisnika, $idL);
            $stmt->execute();
        }
        $stmt->close();
    }

    // INSERT zapisnik_sa_radova_prisutni
    ...
    if ($prisutni) {
        ...
        foreach ($prisutni as $p) {
            ...
            if (!$idTip) continue;
            $stmt->bind_param('iiissi', $id_zapisnika, $idCl, $idTip, $ime, $loza, $idDr);
            $stmt->execute();
        }
        $stmt->close();
    }

    // INSERT zapisnik_sa_radova_duznosnici
    ...
    if ($duznosnici) {
        ...
        foreach ($duznosnici as $dz) {
            ...
            $stmt->bind_param('isi', $id_zapisnika, $naziv, $idCl);
            $stmt->execute();
        }
        $stmt->close();
    }
