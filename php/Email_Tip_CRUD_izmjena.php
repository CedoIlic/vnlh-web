...
// Email_Tip_CRUD_izmjena.php – izmjena tipa e-mail adrese
...
$stmt = $mysqli->prepare("SELECT id FROM email_tip WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE email_tip SET naziv = ?, `Tip` = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
