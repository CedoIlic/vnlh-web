...
// Upis retka šifarnika pred-printa. „izvor_tablica" se prima s bijele liste
// (pdf_dozvoljeni_izvori_dokumenata) i nikad ne ulazi u upit kao ime objekta.
...
if ($id_dokument <= 0 || $izvor_tablica === '') { echo '105'; exit; }

try {
    // Izvor mora biti na bijeloj listi dozvoljenih izvora dokumenata.
    ...
    // Isti dokument iz istog izvora ne smije se ponoviti (UNIQUE uq_kdpp_dokument_izvor).
    $stmt = $mysqli->prepare("SELECT id FROM kandidat_dokumenti_pred_print WHERE id_dokument = ? AND izvor_tablica = ? LIMIT 1");
    if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
    $stmt->bind_param("is", $id_dokument, $izvor_tablica);
    $stmt->execute();
    $stmt->store_result();
    ...
    $stmt = $mysqli->prepare("INSERT INTO kandidat_dokumenti_pred_print (id_dokument, naziv, izvor_tablica, izvor_kolona, redosljed, aktivan, napomena) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
    $stmt->bind_param("isssiis", $id_dokument, $naziv_db, $izvor_tablica, $izvor_kolona, $redosljed, $aktivan, $napomena_db);
    $stmt->execute();
    echo 'OK';
    $stmt->close();
} catch (mysqli_sql_exception $e) {
    if ($e->getCode() == 1452) { echo '107,' . $e->getCode(); exit; }
