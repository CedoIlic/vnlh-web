...
try {
    $stmt = $mysqli->prepare("SELECT id FROM kandidat_dokumenti_sken_tip WHERE LOWER(naziv) = LOWER(?)");
    ...
    $stmt->close();
    $stmt = $mysqli->prepare("INSERT INTO kandidat_dokumenti_sken_tip (naziv, redosljed) VALUES (?, ?)");
    if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
