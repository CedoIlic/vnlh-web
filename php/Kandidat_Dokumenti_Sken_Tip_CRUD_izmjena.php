...
$stmt = $mysqli->prepare("SELECT id FROM kandidat_dokumenti_sken_tip WHERE LOWER(naziv) = ? AND id <> ? LIMIT 1");
...
$stmt->close();
$stmt = $mysqli->prepare("UPDATE kandidat_dokumenti_sken_tip SET naziv = ?, redosljed = ? WHERE id = ?");
if (!$stmt) { echo '200,' . $mysqli->errno; exit; }
