...
// Clanovi_CRUD_izmjena.php – UPDATE tablice clanovi + telefoni / e_maili / adrese (transakcija).
...
try {
    ...
    $sifra = isset($_POST['sifra']) ? trim((string)$_POST['sifra']) : '';
    ...
    $raw_stupanj = isset($_POST['stupanj']) ? trim((string)$_POST['stupanj']) : '';
    $stupanj = ($raw_stupanj === '' || $raw_stupanj === '0') ? null : (int)$raw_stupanj;
    ...
    $zastavice_raw = isset($_POST['zastavice']) ? (int)$_POST['zastavice'] : 0;
    $zastavice = ($zastavice_raw < 0 || $zastavice_raw > 65535) ? 0 : $zastavice_raw;
    ...
    // Jedinstvenost šifre iskaznice unutar države (UNIQUE drzava, sifra u bazi) – ignoriramo trenutni id.
    if ($sifra !== '') {
        $stmt = $mysqli->prepare("SELECT id FROM clanovi WHERE drzava = ? AND sifra = ? AND id <> ? LIMIT 1");
        $stmt->bind_param('isi', $id_drzava, $sifra, $id);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
            $stmt->close();
    ...
    // UPDATE osnovnih podataka u clanovi (bez telefona/e_mail/adresa/upisano).
    $set_parts = [];
    $bind_types = '';
    $bind_values = [];

    $set_parts[] = ($sifra_db === null ? 'sifra=NULL' : 'sifra=?');
    if ($sifra_db !== null) {
        $bind_types .= 's';
    ...
    $set_parts[] = ($stupanj === null ? 'stupanj=NULL' : 'stupanj=?'); if ($stupanj !== null) { $bind_types .= 'i'; $bind_values[] = &$stupanj; }

    $set_parts[] = 'datum_stupnja=?'; $bind_types .= 's'; $bind_values[] = &$datum_stupnja;
    ...
    $set_parts[] = 'kandidat=?'; $bind_types .= 'i'; $bind_values[] = &$kandidat;
    $set_parts[] = 'zastavice=?'; $bind_types .= 'i'; $bind_values[] = &$zastavice;
