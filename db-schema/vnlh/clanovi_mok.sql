...
-- DISKRECIJA: bilješku čita SAMO njezin autor, i to dok je još član iste lože u kojoj ju je zapisao
-- (usporedba `id_loza_upisao` s trenutnom ložom ulogiranog) — prelazak u drugu ložu odsijeca stare bilješke.
-- Iznimka je kontrolna razina (dužnosti iz sustav_varijable 127): ona ČITA sve, ali ne mijenja i ne briše.
-- Izmjena/brisanje: samo autor i samo unutar roka od `datum_upisa` (broj mjeseci u sustav_varijable 128).
...
CREATE TABLE `clanovi_mok` (
  ...
  `id_loza_clan`         int(11) unsigned DEFAULT NULL COMMENT 'Loža ČLANA u trenutku upisa (kopija clanovi.loza) — povijesni zapis, ne prati kasnije prelaske',
  ...
  `upisao_duznost`       int(11) unsigned DEFAULT NULL COMMENT 'Dužnost autora u trenutku upisa (duznosnici.id iz sesije) — informativno, za buduću upotrebu',
  `id_loza_upisao`       int(11) unsigned DEFAULT NULL COMMENT 'Loža AUTORA u trenutku upisa — uspoređuje se s trenutnom ložom čitatelja (uvjet vidljivosti)',
